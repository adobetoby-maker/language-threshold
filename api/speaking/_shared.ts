...
const PRINCIPAL_PATTERN = /^anon_[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i
...
export function applySpeakingCors(req: VercelRequest, res: VercelResponse, methods: string) {
  ...
  if (SPEAKING_ALLOWED_ORIGINS.has(origin) || isDevelopmentOrigin(origin)) {
...
export function requireTrustedOrigin(req: VercelRequest, res: VercelResponse) {
  const origin = typeof req.headers.origin === 'string' ? req.headers.origin : ''
  if (SPEAKING_ALLOWED_ORIGINS.has(origin) || isDevelopmentOrigin(origin)) return true
...
export function requireAnonymousPrincipal(req: VercelRequest, res: VercelResponse) {
  ...
  const principalId = `anon_${randomUUID()}`
  const signed = `${principalId}.${signature(principalId, secret)}`
  const secure = process.env.VERCEL_ENV === 'production' ? '; Secure' : ''
  res.setHeader('Set-Cookie', `${PRINCIPAL_COOKIE}=${encodeURIComponent(signed)}; Path=/api/speaking; HttpOnly; SameSite=Strict; Max-Age=2592000${secure}`)
  return principalId
}
