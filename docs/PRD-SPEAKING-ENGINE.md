...
3. **Speech architecture:** Use a provider-neutral streaming cascade (STT → constrained dialogue/evaluation → TTS) with tap-to-speak as a required fallback and interruption-safe cancellation of in-flight capture/playback. True full-duplex speech-to-speech and barge-in are deferred until a physical-iPhone provider spike proves reliability, retention terms, latency, and cost.
...
6. **Scoring:** `Incomplete` is separate from Clay. Objective completion, understandability, assistance, and communication recovery determine Clay/Bronze/Silver/Gold using a versioned deterministic rubric. For `speaking-rubric-v1`: Clay is a valid ended attempt below Bronze; Bronze requires all critical objectives, at most 3 hints, at most 1 English fallback, at most 3 breakdowns, and at least 55% understandable learner turns; Silver requires all objectives, at most 1 hint, no English fallback, at most 1 breakdown, and at least 70% understandable turns; Gold additionally requires no hints, at least 1 successful recovery, and at least 85% understandable turns. Pronunciation is diagnostic and cannot independently gate mastery. Model judgments require evidence and confidence; circumlocution requires an authored feature threshold and evaluator confidence of at least 0.8.
