...
5. Download the privacy-minimized JSON before Reset. Confirm its privacy flags are all false for content inclusion and that `automaticallyUploaded` is false.
6. Attach the JSON to the controlled review record. Do not paste transcripts into GitHub.
7. Compare provider request/usage records and invoices using request/session IDs. Record only the reconciled totals, not provider keys or speech content.
8. Disable the preview flags and revoke temporary credentials at the end of the test window.

## Required matrix

| Area | Cases | Evidence required |
|---|---|---|
| Provider path | normal turn, natural pause/multiple segments, quiet speech, accented speech, code-switching, domain terms | success/failure, provider request ID, stage timings, usage |
...
- a provider key or signed lease appears in browser-visible output, evidence, logs, or screenshots;
