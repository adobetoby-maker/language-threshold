...
const CLIMBING_APP_URL = `${APP_URL}?module=climbing&lang=de`
      ...
      <div className="max-w-6xl mx-auto">
        <FadeIn>
          ...
          <p className="text-base mb-10" style={{ ...sansFont, color: '#A89F94', maxWidth: '40rem' }}>
            Jedes Ausrüstungsteil hat einen Namen auf Deutsch. Lernen Sie sie, bevor Sie sich einbinden.
          </p>
        </FadeIn>
      ...
      {open && (
        <div className="px-6 pb-6 border-t" style={{ borderColor: `${CLIMBING_COLOR}14` }} onClick={e => e.stopPropagation()}>
          ...
          <div className="rounded-xl p-4 mt-2" style={{ backgroundColor: `${CLIMBING_COLOR}0A`, border: `1px solid ${CLIMBING_COLOR}20` }}>
            <p className="text-xs uppercase tracking-widest mb-2" style={{ ...sansFont, color: CLIMBING_COLOR }}>Beispielsatz</p>
            <p className="text-sm mb-1" style={{ ...sansFont, color: '#F7F3EC' }}>{mod.samplePhrase.en}</p>
            <p className="text-sm italic" style={{ ...sansFont, color: CLIMBING_COLOR }}>{mod.samplePhrase.de}</p>
          </div>
          <p className="text-xs mt-3" style={{ ...sansFont, color: '#6B6460' }}>
            Kontext: {mod.scenario}
          </p>
        </div>
      )}
      {tapped && <WordCard word={tapped.word} sentence={tapped.sentence} x={tapped.x} y={tapped.y} lang="de" color={CLIMBING_COLOR} onClose={() => setTapped(null)} />}
    </div>
  )
}

export default function ClimbingGerman() {
  usePageMeta({ title: 'Climbing German — Klettern auf Deutsch', lang: 'de', canonical: 'https://languagethreshold.com/climbing-german' })
  return (
        ...
        <div className="relative max-w-4xl mx-auto">
          <FadeIn>
            ...
            <h1 className="text-5xl md:text-6xl font-black leading-tight mb-5" style={{ ...displayFont, color: '#F7F3EC' }}>
              Ausrüsten.<br />
              <em className="italic" style={{ color: CLIMBING_COLOR }}>Die Sprache sprechen.</em>
            </h1>
            <p className="text-xl leading-relaxed mb-8" style={{ ...sansFont, color: '#A89F94', maxWidth: '38rem' }}>
              Wichtiges Deutsch für Kletterhallen, Felsen und zweisprachige Seilschaften — Ausrüstungsnamen, Sicherheitskommandos und Vokabular zur Routenlektüre.
            </p>
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              >
                Deutsches Klettern starten →
              </a>
            </div>
          </FadeIn>
          <div className="flex flex-wrap gap-3 mt-10">
            <span key="30+ Ausrüstungsbegriffe" className="text-xs px-3 py-1.5 rounded-full font-medium" style={{ ...sansFont, backgroundColor: `${CLIMBING_COLOR}14`, color: CLIMBING_COLOR, border: `1px solid ${CLIMBING_COLOR}30` }}>
              30+ Ausrüstungsbegriffe
            </span>
            <span key="10 Sicherheitskommandos" className="text-xs px-3 py-1.5 rounded-full font-medium" style={{ ...sansFont, backgroundColor: `${CLIMBING_COLOR}14`, color: CLIMBING_COLOR, border: `1px solid ${CLIMBING_COLOR}30` }}>
              10 Sicherheitskommandos
            </span>
            <span key="3 Themenmodule" className="text-xs px-3 py-1.5 rounded-full font-medium" style={{ ...sansFont, backgroundColor: `${CLIMBING_COLOR}14`, color: CLIMBING_COLOR, border: `1px solid ${CLIMBING_COLOR}30` }}>
              3 Themenmodule
            </span>
            <span key="Alltagsredewendungen" className="text-xs px-3 py-1.5 rounded-full font-medium" style={{ ...sansFont, backgroundColor: `${CLIMBING_COLOR}14`, color: CLIMBING_COLOR, border: `1px solid ${CLIMBING_COLOR}30` }}>
          ...
          <FadeIn>
            ...
            <div className="grid sm:grid-cols-3 gap-5">
              ...
              >
                ...
                <div className="text-lg font-bold mb-1" style={{ ...displayFont, color: '#F7F3EC' }}>Climbing French</div>
                <p className="text-sm" style={{ ...sansFont, color: '#A89F94' }}>The same three modules — for French-speaking climbing partners.</p>
              </a>
              <a
              ...
              <a
                href="/medical-german"
                className="group block p-6 rounded-2xl transition-all"
                style={{ backgroundColor: '#161616', border: '1px solid rgba(239,68,68,0.12)', textDecoration: 'none' }}
              >
                <div className="text-2xl mb-2">🏥</div>
                <div className="text-lg font-bold mb-1" style={{ ...displayFont, color: '#F7F3EC' }}>Medical German</div>
                <p className="text-sm" style={{ ...sansFont, color: '#A89F94' }}>Thirteen clinical modules — German for medical professionals.</p>
              </a>
            </div>
          </FadeIn>
        </div>
      ...
      <section className="py-20 px-6 text-center">
        <FadeIn>
          ...
          <h2 className="text-3xl font-bold mb-4" style={{ ...displayFont, color: '#F7F3EC' }}>
            Bereit zum Klettern in jeder Sprache?
          </h2>
          <p className="text-base mb-8" style={{ ...sansFont, color: '#A89F94' }}>
            Üben Sie Karteikarten, hören Sie die Aussprache und testen Sie sich bei jedem Begriff.
          </p>
          <a
            href={CLIMBING_APP_URL}
            target="_blank"
